...
describe('workers', function () {
  ...
  describe('when prioritized jobs are added', () => {
    ...
    describe('when priority counter is having a high number', () => {
      it('should process jobs by priority', async () => {
        ...
        const jobs = Array.from(Array.from({ length: 18 }).keys()).map(index => ({
          name: 'test',
          data: { p: (index % 3) + 1 },
          opts: {
            priority: (index % 3) + 1,
  ...
  describe('Concurrency process', () => {
    ...
    describe('when changing concurrency', () => {
      describe('when increasing value', () => {
        it('should process job respecting the current concurrency set', async function () {
          ...
          const jobs = Array.from(Array.from({ length: 16 }).keys()).map(index => ({
            name: 'test',
            data: { index },
          }))
      ...
      describe('when decreasing value', () => {
        it('should process job respecting the current concurrency set', async function () {
          ...
          const jobs = Array.from(Array.from({ length: 20 }).keys()).map(index => ({
            name: 'test',
            data: { index },
          }))
    ...
    it('should wait for all concurrent processing in case of pause', async () => {
      let i = 0
      let nbJobFinish = 0
    ...
    describe('when there are delayed jobs between retries', () => {
      describe('when using retryJob', () => {
        it('promotes delayed jobs first', async () => {
          ...
          const jobs = Array.from(Array.from({ length: 3 }).keys()).map(index => ({
            name: 'test',
            data: {},
            opts: {
              delay: 200,
  ...
  describe('Manually process jobs', () => {
    ...
    it('should get paginated unprocessed dependencies keys', async () => {
      ...
      await Promise.all(
        Array.from(Array.from({ length: 65 }).keys()).map((index: number) => {
          return Job.create(
            queue,
            `child${index}`,
            { idx: index, ...value },
      ...
      await Promise.all(
        Array.from(Array.from({ length: 64 }).keys()).map((index: number) => {
          return Job.create(
            queue,
            `child${index}`,
            { idx: index, ...value },
