...
> implements MinimalJob<DataType, ReturnType, NameType> {
  ...
  static fromJSON<T = any, R = any, N extends string = string>(
    ...
    job.delay = Number.parseInt(json.delay)

    job.priority = Number.parseInt(json.priority)

    job.timestamp = Number.parseInt(json.timestamp)

    if (json.finishedOn) {
      job.finishedOn = Number.parseInt(json.finishedOn)
    }

    if (json.processedOn) {
      job.processedOn = Number.parseInt(json.processedOn)
    }
    ...
    job.attemptsStarted = Number.parseInt(json.ats || '0')

    job.attemptsMade = Number.parseInt(json.attemptsMade || json.atm || '0')

    job.stalledCounter = Number.parseInt(json.stc || '0')
  ...
  protected validateOptions(jobData: JobJson) {
    ...
    if (`${Number.parseInt(this.id, 10)}` === this.id) {
...
function getTraces(stacktrace: string[]) {
  ...
  if (traces === errorObject || !(Array.isArray(traces))) {
    return []
  }
  else {
    return traces
