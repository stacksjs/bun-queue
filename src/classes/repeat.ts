...
import type { JobsOptions, RepeatStrategy } from '../types'
...
export class Repeat extends QueueBase {
  ...
  private async getRepeatableData(
    ...
    if (jobData) {
      return {
        key,
        name: jobData.name,
        endDate: Number.parseInt(jobData.endDate) || null,
        tz: jobData.tz || null,
  ...
  async getRepeatableJobs(
    ...
    for (let i = 0; i < result.length; i += 2) {
      jobs.push(
        this.getRepeatableData(client, result[i], Number.parseInt(result[i + 1])),
      )
    }
...
export function getNextMillis(millis: number, opts: RepeatOptions): number | undefined {
